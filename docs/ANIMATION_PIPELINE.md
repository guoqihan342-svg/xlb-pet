...
截至 `2026-08-08`，`v1.0.58` 的运行时打工契约为 `48 / 96 / 96 / 24`，共 264 个逻辑帧；48 帧 `work-tap` 已从图集和程序集移除。整个 Brotli 图集为 53 页、1648 个逻辑帧，但构建仍必须从最终清单动态校验，不能由动作增量手算。
...
`install_generated_motion_assets.py --edge-peek` 会从原始关键姿势中分离两只手和下方前臂的真实连通像素，并把它们作为固定接触层叠回独立探出的头与躯干。肤色手掌只在两个紧椭圆内选取，紫色前臂只在受限多边形内选取；不会复制扫描线、合成新像素或拉伸眼睛和脸。右侧运行时镜像复用左侧，不产生第二套源帧。

历史脚本 `tools/fix_edge_side_arm_reveal.py` 已改成失败关闭迁移桩：旧命令只输出替代流程并以非零状态退出，绝不再改写素材。原来的 7 像素局部 warp 会破坏当前固定接触层并重新引入袖口拉伸。正式 48 帧应满足：上手接触 x 恒为 `0`，下手与前臂仅允许 `0..1px` 的抗锯齿边界，相邻前臂位移不超过 `0.5px`，且图集仍保留 `DestinationX=-2` 的透明 gutter。
