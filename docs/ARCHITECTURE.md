...
- 左键单击会在当前位置停止移动、反向播放下坐骑并回到稳定待机，不再追加四种点击动作。手势阈值使用按下时记录的屏幕物理坐标和当前 DPI；退场期间 `454×454 DIP` HWND 自身移动造成的局部坐标漂移不能触发拖动。
- 只有屏幕物理坐标位移超过按下时 DPI 对应的系统拖动阈值，巡游才立即释放位置所有权并交给捕获式物理拖动；屏幕坐标不可用时，巡游起始手势禁止退回会受窗口移动影响的局部坐标判断。
...
- 同一个无文字 `Button` 语义控件在完全待机且 `EdgeDock.None` 时显示左上角萌太阳，进入非待机打工状态后用 `420 ms` 绝对时间轴交叉切换为萌月亮；两个纯 XAML 矢量图标始终保留在视觉树中，只更新预创建的透明度和 Transform，外层命中区保持 42 DIP。退出请求被接受的同一渲染帧即反向，快速反转从当前混合状态继续；总图标透明度约束为 `1.00–1.05`，避免空白帧、闪白和双影。该切换复用人物的唯一 `CompositionTarget.Rendering + Stopwatch`，不创建 Storyboard、额外 Timer、位图或逐帧对象。普通吸附隐藏太阳；打工拖动命中边缘后会直接请求边缘交接，不创建保留月亮的“工作吸附”中间态。人物水平镜像时布局和图标自身反向补偿，最终仍固定在屏幕视觉左上角且按钮事件不能冒泡成普通人物点击。
