...
- 左键单击会在当前位置停止移动、反向播放下坐骑并回到稳定待机，不再追加六种点击动作。
...
打工是用户主动进入的常驻状态，不属于六种普通点击动作，也不写入设置文件。主状态与打字子阶段的交接为：
...
- `v1.0.60` 的当前打工资源契约共 264 个逻辑帧：`work-enter`、`work-loop`、`work-serious-loop`、`work-serious-exit` 分别为 48、96、96、24 帧；`work-tap` 已从运行时、程序集和图集移除，两种循环仍保留 9 个精确中性接缝和 65 张独特位图。普通挥手动作亦已从运行时、程序集和图集移除，整套目标图集为 48 页、1502 个逻辑帧；最终页数、帧数和动作长度必须由同次构建的清单动态校验。
...
| 六种普通动作及挥手不进入运行时图集 | `--atlas-hash-only`、素材 QA |
