...
| `tools/build_work_animation.py` | 从 v5 中性锚点、v6 无臂底图与左右预抬参考确定性生成 `48/96/48/96/24` 打工序列及 QA |
...
- 打工普通/认真循环必须保持 96 个逻辑帧、至少 70 张独特位图、8 个精确中性接缝和最长 3 帧连续相同画面；普通循环为 1.6 秒并包含 6 次强触键与 2 次轻微换位。v6 完整双臂关节链必须满足肩 `<` 肘 `<` 腕 `<` 指尖的位移层级，目标指尖保持在 `5.5–9.5 px` 验收区间，脸、电脑和双臂允许区域外必须零漂移。
- 打工单击只允许在最近精确中性接缝播放敲头后回到普通 1 倍速。双击立即把当前相位切到 2 倍速，到最近精确接缝后播放约 133 ms 认真眉过渡，再从认真循环开始完整计时 4 秒；不得用临时加速补播积压帧或在触键途中硬切。
