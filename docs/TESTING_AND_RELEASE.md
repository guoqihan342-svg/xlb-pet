...
| `--work-mode-only` | `48/96/48/96/24` 序列、普通 1.6 秒、两种循环各 81 张独特位图、8 个精确中性接缝、最长 3 帧连续相同画面、v6 完整双臂联动、单击敲头后回普通 1 倍速、双击立即 2 倍速并在最近接缝经约 133 ms 认真眉过渡后完整计时 4 秒、认真退出、边缘入口与交互抢占 |
...
| `--work-preview` | 头顶胶囊按钮、v6 肩到指尖的完整双臂关节运动、普通 6 次强触键加 2 次换位、单击只敲头后回普通速度、双击连续加速到最近接缝并切换认真眉、左/右/下吸附退出后开工与收工 |
...
- 完全待机时可通过“去打工”进入电脑场景。普通 96 帧循环必须在 1.6 秒内显示 6 次强触键和 2 次轻微换位，最长连续相同画面不超过 3 帧；肩、肘、袖口、手腕、掌根和手指应逐级自然跟随，专用眉形补丁之外的脸、电脑和双臂允许区域外不得漂移。工作中单击只播敲头并回普通 1 倍速；双击不敲头，必须立即连续切到 2 倍速，在最近精确中性接缝经约 133 ms 认真眉过渡后再完整保持至少 4 秒；“下班啦”在收工完成前保持正确状态，随后回到稳定待机。
...
以下结果于 `2026-08-05` 在 Windows 11 x64、.NET SDK 8、.NET Desktop Runtime `8.0.29` 环境完成。GitHub Release 指向正式源码提交；随后仅追加本节发布证据。
