...
| `--clip-clock-only` | 单缓冲预乘 Alpha、冷页时钟、普通动作绝对时间轴，以及蝴蝶飞行/悬停/扇翅的同钟定位与中断清理 |
...
独立蝴蝶还应校验 `tools/generated_sources/luban-butterfly-v1-green.png` 的来源存在，以及正式 `Assets/luban-butterfly.png` 为 `96×96` RGBA、Alpha 非空、透明 RGB 为零、无绿色溢边且四边至少保留 6 px 透明 gutter。蝴蝶是明确白名单的 WPF Resource，不得被误收进人物图集。

详细生成顺序和不变量见 [动画与图集管线](ANIMATION_PIPELINE.md)。`v1.0.62` 必须断言普通点击与随机袋严格只含 `butterfly / cry / cute / like / eat`；butterfly 复用 `think` 人物像素但 ActionName、对白和状态所有权都不能冒充 Todo。Todo 的完整 56 帧 `think` smooth 入场和稳定托腮姿势必须继续存在。最终图集、程序集和发布 EXE 不得包含 `yawn`、`loop-cute`、`cute-smooth-057..090`、`loop-think` 页面、48 帧普通思考循环或普通 `luban-wave-*` 运行时资源；`pic/小鲁班1.jpg` 与 `pic/小鲁班8.png` 必须继续保留。目标清单为 41 页、1240 帧，仍需由正式构建动态复核。
...
- 五种点击动作从待机连续起身并返回；顺序为蝴蝶、委屈、欢呼卖萌、点赞、吃饼干，第六次有效点击回到蝴蝶，自动随机袋也只含这五种动作与待机。蝴蝶必须飞近鼻尖、原地扇翅停留后飞走，人物复用高清 `think` 姿势且不出现独立人物分页；点击、拖动、右键或高优先级状态接管后，蝴蝶必须在同一视觉交接中消失。打哈欠、普通“让我认真想一想……”和“嗨～我在这里！”挥手不得再出现；右键打开任务面板仍须完整进入并保持 Todo 专用托腮思考姿势；稳定待机只有一个半透明泡泡。
