...
| `--work-mode-only` | `48/96/96/24` 序列、普通 1.6 秒、65 张独特循环位图、9 个精确中性接缝、单击严格无操作、双击认真表情、打工拖动与左/右/下平滑收工交接、顶部与共享接缝拒绝吸附、左上角太阳/月亮矢量入口、镜像补偿及交互抢占 |
...
| `--work-preview` | 左上角萌太阳/月亮、v5 语义手/袖口关节运动、8 次不等间隔四指落键、单击无额外动作、双击认真表情、打工中拖动保持动画、命中左/右/下后平滑收工并交接真实探头，且全程不闪待机中间帧 |
...
详细生成顺序和不变量见 [动画与图集管线](ANIMATION_PIPELINE.md)。`v1.0.61` 必须断言普通点击与随机袋严格只含 `yawn / cry / cute / like / eat`，普通 `think` 与 `wave` 均无入口；Todo 的完整 `think` smooth 入场和稳定托腮姿势必须继续存在。最终图集、程序集和发布 EXE 不得包含 `loop-think` 页面、48 帧普通思考循环或普通 `luban-wave-*` 运行时资源；`pic/小鲁班8.png` 必须继续保留。
...
- 五种点击动作从待机连续起身并返回；第六次有效点击回到第一种，自动随机袋也只含这五种动作与待机。普通“让我认真想一想……”和“嗨～我在这里！”挥手不得再出现；右键打开任务面板仍须完整进入并保持 Todo 专用托腮思考姿势；稳定待机只有一个半透明泡泡。
...
- 打工期间不启动自动动作、呼噜或绕屏；拖动过程应继续当前动画与绝对相位。松手命中左、右或下外边缘时，必须锁定目标边缘、平滑播完收工、不闪趴枕头中间帧，再进入同一边缘的真实 `EdgePeek`；顶部及双屏内部接缝不触发收工交接。右键和定时提醒接管时不得留下迟到的边缘请求。
...
本节先记录本轮发布目标，不能作为已经完成的验证证据。代码、图集、自动化、正式 EXE、实机冒烟、GitHub 标签与 Release 附件均须在最终发布后补入实际命令结果、提交、大小和 SHA-256。

| 项目 | 目标与当前状态 |
| --- | --- |
| 源码与版本 | 待完成：版本目标 `1.0.61`，正式功能提交尚未产生 |
| 五种普通动作 | 待完成：点击循环与空闲随机袋应严格为 `yawn / cry / cute / like / eat`，均不得包含普通 `think` 或 `wave` |
| 右键 Todo 思考 | 待完成：完整 `idle → wake → think` 入场、稳定托腮姿势和反向退场必须保留，且不得重新进入普通点击或随机袋 |
| 图集边界 | 待完成：删除 `loop-think` 1 页、48 帧后，目标清单为 47 页、1454 个源帧和 1454 个分页帧；正式数量仍以同次构建结果为准 |
| 打工三边交接 | 待完成：沿用 `v1.0.60`，左/右/下平滑收工后进入真实 `EdgePeek`，顶部和共享接缝不吸附 |
| 自动化与素材 QA | 待完成 |
| EXE、实机与用户数据 | 待完成 |
| GitHub Release | 待完成：`v1.0.61` 尚未发布 |
