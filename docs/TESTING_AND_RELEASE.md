...
详细生成顺序和不变量见 [动画与图集管线](ANIMATION_PIPELINE.md)。`v1.0.60` 还必须断言六种普通动作清单不含 `wave`，最终图集、程序集和发布 EXE 不得包含 `luban-wave-*` 运行时资源；`pic/小鲁班8.png` 必须继续保留。
...
- 六种点击动作从待机连续起身并返回；第七次有效点击回到第一种，自动随机袋也只含这六种动作与待机。“嗨～我在这里！”挥手不得再出现；稳定待机只有一个半透明泡泡。
...
本节只记录发布门槛，不把目标写成已通过。正式构建、自动回归、真实 WPF 实机、EXE 哈希、标签和 Release 附件完成后，再用实际结果替换“待回填”。

| 项目 | 发布门槛 |
| --- | --- |
| 源码版本 | `v1.0.60`；正式提交 SHA 待回填 |
| 六种普通动作 | 点击与自动随机袋仅包含 `yawn / cry / cute / like / eat / think`；“嗨～我在这里！”不得播放 |
| 挥手资源边界 | `pic/小鲁班8.png` 保留；代码、清单、程序集、最终图集和新发布 EXE 均不含 `luban-wave-*` 运行时帧 |
| 图集 | 移除普通挥手 5 页 / 146 帧后的目标为 48 页 / 1502 帧；必须从实际清单和解码 QA 回填，不得只凭手算判定通过 |
| 打工三边交接 | 普通与认真打字分别拖到左/右/下：松手后平滑收工，再进入真实探头；方向、镜像、对齐正确，不闪待机帧，不残留月亮或工作动画 |
| 多屏反例 | 顶部、双屏内部接缝、距边缘超出阈值都不得触发收工交接；负坐标与混合 DPI 外边缘必须命中正确显示器 |
| 其他回归 | 待办、定时任务、提醒、巡游、普通三边探头、桌宠大小和开机自启保持兼容 |
| EXE 与 GitHub Release | 字节数、文件版本、产品版本、SHA-256、签名状态、标签、Release 目标提交与独立回下载哈希均待回填 |
