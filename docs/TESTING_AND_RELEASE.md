...
| `--star-wish-only` | `star-wish` 复用 `cute` 56 帧、`96×96` 唯一星星 Resource、常驻透明 `22 DIP` overlay、绝对轨迹、抢占清理以及 41 页/1240 帧图集不变 |
| `--clip-clock-only` | 单缓冲预乘 Alpha、冷页时钟、普通动作绝对时间轴，以及 `star-wish` 对既有 `cute` 人物时间线的复用和中断清理 |
| `--work-mode-only` | `48/96/96/24` 序列、普通 1.6 秒、65 张独特循环位图、9 个精确中性接缝、单击严格无操作、双击认真表情，以及打工拖动命中左/右/下时跳过普通退出与 idle 的热页/冷页原子交接 |
...
| `--star-wish-preview` | 真实第一条 reaction：复用 `cute` 举手姿势，一颗小星星从画面右侧的小手边沿帽檐外缘升起、在头顶轻摇后飞走；用于人工确认只有一颗星、轨迹连续且抢占后立即隐藏 |
| `--work-preview` | 左上角萌太阳/月亮、v5 语义手/袖口关节运动、8 次不等间隔四指落键、单击无额外动作、双击认真表情，以及打工拖动命中左/右/下后直接交接真实探头，全程不播普通退出、枕头或待机中间帧 |
...
许愿星的正式图像源清单只有 `Assets/luban-wish-star.png`。应确认它为 `96×96` RGBA 透明 PNG、画面只有一颗星、文件作为 WPF Resource 嵌入；`WishStarOverlay` 必须常驻视觉树、大小为 `22 DIP`、初始 `Opacity=0` 且 `IsHitTestVisible=False`。旧 `Assets/luban-butterfly.png`、失败的全人物 `star-cuddle` 帧与生成源、对应项目 Resource、人物图集页和运行时专用代码均应不存在。

详细生成顺序和不变量见 [动画与图集管线](ANIMATION_PIPELINE.md)。`v1.0.63` 必须断言普通点击与随机袋严格只含 `star-wish / cry / cute / like / eat`；`star-wish` 以独立 ActionName 复用 `cute` 的 56 张正播、既有 wake 桥和自然反播，不拥有独立人物 smooth 或 loop。星星轨迹必须由 `CompositionTarget.Rendering + Stopwatch` 绝对时间驱动，一颗星依次完成画面右侧手边沿帽檐外缘升起、头顶轻摇和飞走；59/60/120/144 Hz 的同一绝对时间应得到同一位置、缩放、旋转和透明度。Todo 的完整 56 帧 `think` smooth 入场和稳定托腮姿势必须继续存在。最终图集、程序集和发布 EXE 不得包含 butterfly ActionName、旧中文对白、失败的 `star-cuddle` 人物帧或图集页，也不得包含 `yawn`、`loop-cute`、`cute-smooth-057..090`、`loop-think` 页面、48 帧普通思考循环或普通 `luban-wave-*` 运行时资源；`pic/小鲁班1.jpg` 与 `pic/小鲁班8.png` 必须继续保留。最终清单必须保持 41 页、1240 个源帧和 1240 个分页帧。
...
不要从 README 或旧测试复制其他版本的固定帧数。清单、实际分页、嵌入资源和源集指纹必须来自同一次构建；`v1.0.63` 的轻量 `star-wish` 不新增人物分页，因此本次正式清单必须仍为 41 页、1240 个源帧和 1240 个分页帧。
...
- 五种点击动作从待机连续起身并返回；顺序为许愿星、委屈、欢呼卖萌、点赞、吃饼干，第六次有效点击回到许愿星，自动随机袋也只含这五种动作与待机。许愿星人物必须与现有 `cute` 56 帧及自然反向退场逐帧一致；画面中只能有一颗约 `22 DIP` 的小星星，从画面右侧的小手边沿帽檐外缘连续升起、在头顶轻摇后飞走，不能出现闪现、断轨、命中区域、人物缩放抖动或额外全人物分页。点击、拖动、右键或高优先级状态应正常接管并立即隐藏 overlay。旧蝴蝶对白与资源、失败的 144 帧星星方案、打哈欠、普通“让我认真想一想……”和“嗨～我在这里！”挥手不得再出现；右键打开任务面板仍须完整进入并保持 Todo 专用托腮思考姿势；稳定待机只有一个半透明泡泡。
