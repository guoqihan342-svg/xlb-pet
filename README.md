...
- 每行左侧的 `≡` 是专用拖拽手柄；从手柄拖动即可调整顺序，不会抢占正文的鼠标选择和 `Ctrl+C`。每条待办最多输入 5000 字。只有正文确实被当前行裁掉时，鼠标悬停才会在左侧显示“待办完整内容 · 可选择复制”；左侧空间不足才改到右侧，短内容不弹窗。行内只允许选择当前看得到的文字，拖到行外不会偷偷向下滚动。单击每行右侧笔尖朝左下的铅笔图标，会打开固定 `378×414 DIP` 的浅蓝萌系编辑窗，优先放在面板左侧、空间不足时放到右侧；只有点击“确定修改”才保存，点击其他位置不会自动保存，`Esc` 取消。`F2` 仍可进入兼容的行内修改。空白内容不会覆盖原待办，微软输入法仍在选词时会等候文字正式上屏再处理。
