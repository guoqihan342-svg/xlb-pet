...
- 左键单击：每次只播放一个约 0.8–1.1 秒的短动作，随后立即回到睡觉待机，不会再把所有动作串成一条长动画。
- 8 个点击动作按“打哈欠 → 委屈 → 奔跑 → 欢呼 → 点赞 → 吃饼干 → 挥手 → 思考”的顺序轮换；第 9 次有效点击重新从打哈欠开始。
- `pic` 文件夹中的 9 张原图会跨多次点击全部用上：`小鲁班2.png` 提供固定睡觉待机，其余 8 张分别对应 8 个短动作。
- 每个短动作只使用自己的 1–2 张关键姿势桥接图，从睡觉姿势沿同一肢体轨迹进入动作，再按相反顺序返回待机；相邻姿势采用 90ms 双图层交叉淡入，动作图在淡入完成后再按自身节奏停留。
...
8 个点击动作分别拥有独立的待机桥接序列。哈欠、委屈、奔跑、卖萌、点赞和挥手使用两张连续关键姿势；吃饼干与思考使用一张即可自然衔接。每次点击固定只经过“待机 → 本动作桥接序列 → 本动作 → 反向桥接序列 → 待机”，不会夹入其他动作的姿势。挥手全程保持同一只手，点赞保持同一只拇指和起身轨迹，枕头也会沿桥接姿势落到地面而不是突然换位。全部运行时资源均为 `900×1100 RGBA` 透明 PNG，人物中心与底部锚点统一，并使用高质量缩放。
