...
- 每组点击动作都会经过“趴枕头待机 → 连续起身 → 专属动作 → 原路返回待机”。姿势以 60fps 目标节拍播放，起身、动作入口和变化较大的动作内部使用经过连续性检查的补间姿势；实际帧数由最终资源序列和图集清单动态决定。挥手只使用没有多手重影的前 40 帧；“欢呼卖萌”把原先第二次上下弹跳的帧预算重新分配给第一次蹲起，在前 56 帧完成一次连续下蹲、回弹和张嘴笑，不再重复抖身高。两者在末姿势停留约 500ms 后返回，其余五组继续使用完整循环段。
...
- 每行左侧的 `≡` 是专用拖拽手柄；从手柄拖动即可调整顺序，不会抢占正文的鼠标选择和 `Ctrl+C`。每条待办最多输入 5000 字；鼠标停在一行上会显示可滚动、可选中并可用 `Ctrl+C` 复制的完整正文。单击每行右侧笔尖朝左下的铅笔图标，会打开初始约 `6cm × 3.708cm` 的白色可爱编辑窗，可拖动、可缩放，鼠标点到编辑窗外自动保存；`Ctrl+Enter` 保存、`Esc` 取消。`F2` 仍可进入兼容的行内修改。空白内容不会覆盖原待办，微软输入法仍在选词时会等候文字正式上屏再保存。
...
- 待办和定时任务正文均最多输入 5000 字，列表悬停浮层可滚动、选中并复制全文。单击定时任务右侧铅笔会先打开同样的白色可缩放正文编辑窗；需要改日期、时分秒或循环规则时，再点“时间与循环…”进入完整表单。只改正文会原样保留下一次时间和循环进度；修改规则不会偷偷以当前保存时刻重新计时。“保存”会保留任务身份、重新排序并重新调度，“取消”、切换选项卡或关闭窗口都不会误改原任务。待提醒列表自动保存到 `%LocalAppData%\LubanDesktopPet\scheduled-tasks.json`。
...
- 到点提醒会立即中断绕屏并移动到人物当前显示器的右下角。已到点任务按“到期时间 → 创建时间 → 标识”从上到下合并显示；在点击“都知道啦”前新到点的任务会继续加入同一个对话框，一次确认统一完成当前可见批次。程序休眠、界面短暂阻塞或到点时未运行，都不会快速补播计时；下次可用时会立即检查所有逾期任务。逾期超过 5 秒时，对话框会显示“已错过 N 次”：单次任务为 1 次，循环任务按持久化的原始节奏精确统计，不会连续弹出多个补偿窗口。
- 到点不会立刻从磁盘删除任务，只有用户点击“知道啦”后才原子保存结果。单次任务会删除；循环任务会沿原来的时间节奏直接推进到第一个未来时刻并继续保留，推进后的下一次时间会立即持久化，所以重启后不会失灵或重复补播。提醒期间退出或崩溃时，尚未确认的内容会在下次启动重新提示，不会静默丢失。
...
- 日志记录应用启动来源、开机自启设置结果、应用退出、动作开始/结束、熊猫坐骑巡游开关与开始/结束摘要、边缘探头、待办状态、定时任务标识、循环方式、触发时间与错过次数以及未处理异常，不会记录待办或提醒正文，也不会在每个巡游渲染帧写日志。
...
- [ ] 快速往返拖动大小滑块，人物和滑块都连续；待办与定时任务均验证 5000 字输入、浅蓝色行悬停、全文悬浮选择复制、初始 `6cm × 3.708cm` 白色编辑窗的拖动/缩放/外点保存，以及微软/搜狗输入法。
- [ ] 新建并修改普通秒级提醒，确认自定义日期月历的跨月/跨年/闰年/今天选择、完整 `HH:mm:ss`、三列下拉值选择、表单回填、取消、保存后的重新排序和重新调度正确；逐一点击时、分、秒后外层时间窗不得关闭，勾选循环或切换单位也不得关闭，只有“确定”、`Esc` 或外点才关闭。再验证每小时第 11 分 10 秒、每 3 小时第 11 分 10 秒、每天 11:11:11、每 N 天 11:11:11 和每 N 小时第 22 分 00 秒，退出并重启后规则、下一次时间与循环序号仍存在。把循环任务模拟为离线错过多次，确认只弹一次且显示准确的“已错过 N 次”，点击确认后仍保留该任务并沿原节奏推进到未来。最后设置先后到点的多条提醒和长文字提醒，确认绕屏被立即抢占、人物在当前屏右下平滑放大、内容按顺序合并且正文可复制，最后恢复原尺寸且 `settings.json` 未写成 140%。
- [ ] 开启、关闭开机自启并重启程序，确认只修改当前用户 Run 键中的 `LubanDesktopPet` 值；移动 EXE 后手动运行一次，确认旧路径被修复。测试代码必须使用内存假后端，不得读写真实 Run 键。
- [ ] 在新增待办和行内编辑中分别选中第一个字符按 `Ctrl+X`，确认剪贴板得到该字符、文字只删除一次且光标回到索引 0；模拟剪贴板被短暂占用时，首次不得丢字，延迟重试只有在全文和选区都未变化时才删除。
