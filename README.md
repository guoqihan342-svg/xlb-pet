...
- 待办窗口提供“收起”“退出”“自动绕屏移动”和桌宠大小滑块。默认逻辑尺寸为 `190×242`，比旧版高度增加约 1.5 厘米；可在 75%～140% 之间调节并自动保存。所有姿势共用同一尺寸基准，打开或收起待办不会让人物忽大忽小。
...
- 291 个逻辑帧按 `190×242` 原生显示基准离线整理为 13 张紧凑分页图集；只裁掉完全透明的空白，每个精灵保留 2 像素透明隔离带，人物像素和透明通道不降低。
- 运行时只复用一个分页缓冲和一个固定 `190×242 Pbgra32` 完整帧缓冲。每一帧都先写入相同大小、相同坐标的显示面，状态切换再在这一张缓冲内做预乘 Alpha 插值，不叠加两个半透明人物，因此既避免布局重采样的光纹，也不会出现旧帧双重亮边。
- 跑步使用 16 相位双交叉循环；边缘探头放慢为 220ms 步进并在端点停留 500ms。活跃过渡跟随屏幕刷新率更新，绕屏位置按显示器物理像素对齐。
