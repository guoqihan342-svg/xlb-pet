...
- 每组点击动作都会经过“趴枕头待机 → 连续起身 → 专属动作 → 原路返回待机”。姿势以 60fps 目标节拍播放，起身、动作入口和变化较大的动作内部使用经过连续性检查的补间姿势；实际帧数由最终资源序列和图集清单动态决定。挥手只使用没有多手重影的前 40 帧，“欢呼卖萌”只使用身高稳定的前 56 帧，两者在末姿势停留约 500ms 后返回；其余五组继续使用完整循环段。
