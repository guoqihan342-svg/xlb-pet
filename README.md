...
- 左键拖动可以移动小鲁班。人物可见边缘进入当前显示器工作区左、右或下边界的 `12 DIP` 磁吸区，或快速拖动稍微越过外边缘后松手，都会可靠吸附并进入探头状态；超过磁吸区时不会提前触发。顶部中央不吸附、不播放边缘动作；拖到左上角或右上角时仍分别按左、右边缘处理。
...
- 在右键窗口切换到“定时任务”，新草稿默认带入当前本地日期与整秒。点击日期框会打开固定六周的暖橙色圆角月历，支持上月、下月、跨年、闰年、今天快捷选择与相邻月份日期；点击时间框则用时、分、秒三列选择器调整，完整显示 `HH:mm:ss`，下拉值会显式选中并立即收起。单击任务右侧与关闭按钮同风格的铅笔图标，可把内容和时间回填到表单中修改；“保存”会保留任务身份、重新排序并重新调度，“取消”、切换选项卡或关闭窗口都不会误改原任务。待提醒列表自动保存到 `%LocalAppData%\LubanDesktopPet\scheduled-tasks.json`。
...
- 到点提醒会立即中断绕屏并移动到人物当前显示器的右下角。已到点任务按“到期时间 → 创建时间 → 标识”从上到下合并显示；在点击“都知道啦”前新到点的任务会继续加入同一个对话框，一次确认统一完成当前可见批次。程序休眠、界面短暂阻塞或到点时未运行，都不会快速补播计时；下次可用时会立即检查所有逾期任务。
...
- [ ] 新建并修改普通秒级提醒，确认自定义日期月历的跨月/跨年/闰年/今天选择、完整 `HH:mm:ss`、三列下拉值选择、表单回填、取消、保存后的重新排序和重新调度正确；再设置先后到点的多条提醒和长文字提醒，确认绕屏被立即抢占、人物在当前屏右下平滑放大、内容按顺序合并且正文可复制，一次点击“都知道啦”后才删除当前批次，最后恢复原尺寸且 `settings.json` 未写成 140%。
