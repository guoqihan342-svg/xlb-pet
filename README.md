...
- 启动时只同步解码首个待机页，随后仅后台预热定时提醒、四边探头和起身前两页；用户立即触发的动作仍可抢占普通预热。解码页使用代码内固定的 `144 MiB` LRU 软预算，待机、提醒与探头热页，以及当前页、待显示页和当前完整动作涉及的分页都不会被驱逐；若保护集合临时超过预算，画面完整性优先。动作结束后缓存会裁到 `96 MiB` 待机目标；累计释放至少 `48 MiB` 大页时，只在无动作、提醒、拖动、缩放、待办或分页解码的空闲窗口延迟请求一次非压缩 Gen2，且两次至少间隔 30 秒。它不在 Rendering 回调或动作中执行，也不压缩 LOH，不降低像素、帧数或采样质量；总进程内存仍会随 Windows、DPI、CLR 和显卡驱动变化。
...
- [ ] 等关键分页预热结束后复测动作，确认 resident 解码页动作期不超过 `144 MiB`、待机后回落到 `96 MiB`、受保护页优先、LRU 冷页能重新载入；连续动作压力下 Gen2 间隔不少于 30 秒且暂停不超过 50 ms，进程内存没有持续增长；`log` 中没有未处理异常、Brotli 解码失败或分页预热失败。
