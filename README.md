...
- 在右键窗口切换到“定时任务”，新草稿默认是单次提醒并带入当前本地日期与整秒。点击日期框会打开固定六周的暖橙色圆角月历，支持上月、下月、跨年、闰年、今天快捷选择与相邻月份日期；点击时间框则用时、分、秒三列选择器调整，完整显示 `HH:mm:ss`。逐列选择时只收起当前列的下拉列表，外层“选择提醒时间”窗口会一直保留，直到点击“确定”、按 `Esc` 或点击外部。勾选“循环”后仍保留日期与时分秒，并可设置每多少天、小时、分钟提醒一次；所选时间作为新任务的首次提醒时间或修改任务的下一次提醒时间，之后按设定间隔循环。单击任务右侧与关闭按钮同风格的铅笔图标，可把内容、所选时间和循环间隔回填到表单中修改；修改间隔不会自动改变已经选好的下一次提醒时间。“保存”会保留任务身份、重新排序并重新调度，“取消”、切换选项卡或关闭窗口都不会误改原任务。待提醒列表自动保存到 `%LocalAppData%\LubanDesktopPet\scheduled-tasks.json`。
...
- [ ] 新建并修改普通秒级提醒，确认自定义日期月历的跨月/跨年/闰年/今天选择、完整 `HH:mm:ss`、三列下拉值选择、表单回填、取消、保存后的重新排序和重新调度正确；再创建天/小时/分钟循环提醒，退出并重启程序，确认间隔和下一次时间仍存在。把循环任务模拟为离线错过多次，确认只弹一次且显示准确的“已错过 N 次”，点击确认后仍保留该任务并沿原节奏推进到未来。最后设置先后到点的多条提醒和长文字提醒，确认绕屏被立即抢占、人物在当前屏右下平滑放大、内容按顺序合并且正文可复制，最后恢复原尺寸且 `settings.json` 未写成 140%。
