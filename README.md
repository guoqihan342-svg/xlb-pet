...
- 四个方向都有对应姿势。小鲁班会从只露一点的好奇姿势逐步探出，交替抓紧边缘、歪头、眨眼并露出一点肩膀；完全探出和害羞收回都保留卖萌停顿。背景透明，不绘制黑墙或黑色挡板。
...
- 人物动作和手动边缘探头由单一 `CompositionTarget.Rendering + Stopwatch` 绝对时间轴驱动。边缘探头使用 `48` 帧闭环（四阶段各 `12` 帧）、原生 60fps 姿势节拍和 500ms 卖萌端点，不受点击动作的全局倍速影响，避免在 59/60Hz 屏幕周期性跳姿势；窗口呈现跟随显示器合成刷新，回调稍晚时直接定位正确姿势，不快速补播积压帧。相邻姿势直接发布清晰单帧，较大变化由专用桥接姿势连接，不做整图交叉淡化。
