...
- 在右键窗口切换到“定时任务”，新草稿默认是单次提醒并带入当前本地日期与整秒。点击日期框会打开固定六周的暖橙色圆角月历，支持上月、下月、跨年、闰年、今天快捷选择与相邻月份日期；点击时间框则用时、分、秒三列选择器调整，完整显示 `HH:mm:ss`，下拉值会显式选中并立即收起。勾选“循环”后可设置每多少天、小时、分钟提醒一次，首次提醒时间为保存时刻加上该间隔。单击任务右侧与关闭按钮同风格的铅笔图标，可把内容、单次时间或循环间隔回填到表单中修改；只改循环任务文字不会重置下一次时间，修改间隔才从保存时刻重新计时。“保存”会保留任务身份、重新排序并重新调度，“取消”、切换选项卡或关闭窗口都不会误改原任务。待提醒列表自动保存到 `%LocalAppData%\LubanDesktopPet\scheduled-tasks.json`。
