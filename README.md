...
- 在右键窗口切换到“定时任务”，新草稿默认带入当前本地日期与整秒；点击圆角时间框即可用时、分、秒三列选择器调整。单击任务右侧与关闭按钮同风格的铅笔图标，可把内容和时间回填到表单中修改；“保存”会保留任务身份、重新排序并重新调度，“取消”、切换选项卡或关闭窗口都不会误改原任务。待提醒列表自动保存到 `%LocalAppData%\LubanDesktopPet\scheduled-tasks.json`。
...
- [ ] 新建并修改普通秒级提醒，确认当前整秒默认值、三列时间选择器、表单回填、取消、保存后的重新排序和重新调度正确；再设置先后到点的多条提醒和长文字提醒，确认绕屏被立即抢占、人物在当前屏右下平滑放大、内容按顺序合并且正文可复制，一次点击“都知道啦”后才删除当前批次，最后恢复原尺寸且 `settings.json` 未写成 140%。
