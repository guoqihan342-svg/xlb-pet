...
- 每行左侧的 `≡` 是专用拖拽手柄；从手柄拖动即可调整顺序，不会抢占正文的鼠标选择和 `Ctrl+C`。每条待办最多输入 5000 字。只有正文确实被当前行裁掉时，鼠标悬停才会在左侧显示“待办完整内容 · 可选择复制”；左侧空间不足才改到右侧，短内容不弹窗。行内只允许选择当前看得到的文字，拖到行外不会偷偷向下滚动。单击每行右侧笔尖朝左下的铅笔图标，或选中正文后按 `F2`，都会打开同一个固定 `378×414 DIP` 的浅蓝萌系编辑窗，优先放在面板左侧、空间不足时放到右侧；只有点击“确定修改”才保存，点击其他位置不会自动保存，`Esc` 取消。空白内容不会覆盖原待办，微软输入法仍在选词时会等候文字正式上屏再处理。
...
- 待办窗口提供“收起”“退出”、默认勾选的“绕屏动画”、默认关闭的“开机自启”和桌宠大小滑块。开机自启只使用当前用户的 `HKCU\Software\Microsoft\Windows\CurrentVersion\Run`，不需要管理员权限，也不写入 `settings.json`；每次启用、停用和旧路径修复都会立即读回校验，失败则恢复原值。移动 EXE 后手动运行一次会修复已启用的旧路径。绕屏勾选直接位于尺寸滑块上方，只控制自动熊猫坐骑巡游。默认逻辑尺寸为 `190×242`，可在 75%～140% 之间连续调节。待办面板会在人物进入动画前用一次原子窗口提交准备透明缩放包络，并在面板存续期间固定复用；点击滑轨和拖动表情圆钮都只更新同一个合成缩放，不会把动画换帧与透明窗口扩缩挤在同一帧。同值点击不创建新的预览会话，连续点击也不会反复扩张、收回。缩放预览由屏幕合成时钟平滑驱动，停止调节后只保存设置；收起面板时会先在当前姿势冻结的一刻一次性提交最终原生窗口尺寸，再连续播放退出动画，避免末帧抖动、频繁重排窗口和写磁盘。所有姿势共用同一尺寸基准，打开或收起待办不会让人物忽大忽小。
...
- 待办和定时任务正文均最多输入 5000 字，全文悬停规则与待办一致。单击定时任务右侧铅笔会打开固定 `378×414 DIP` 的橘色独立编辑窗，优先放在面板左侧、空间不足时回退右侧；正文、日期、时、分、秒和循环规则一次回填、一起修改。修改窗与新增表单使用同款 `yyyy-MM-dd` 暖橙月历和 `HH:mm:ss` 三列时间组件；依次选择时、分、秒，或勾选循环、修改次数、切换循环单位时，外层选择时间窗都会保持，`Esc` 先收起选择窗而不取消整个修改。内容较多时只在窗内平滑滚动。只有点击“确定修改”才提交并保留任务身份，然后重新排序和调度；点击窗外不会自动保存，没有打开内部选择窗时按 `Esc` 会取消且不会误改原任务。待提醒列表自动保存到 `%LocalAppData%\LubanDesktopPet\scheduled-tasks.json`。
...
- [ ] 快速往返拖动大小滑块，人物和滑块都连续；待办与定时任务均验证 5000 字输入、短行不弹全文窗、长行左优先全文选择复制、固定 `378×414 DIP` 编辑窗的左优先/右回退、浅蓝待办编辑、橘色定时编辑、点击窗外不保存、仅“确定修改”保存、`Esc` 取消，以及微软/搜狗输入法。
