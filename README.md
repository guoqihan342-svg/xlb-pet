...
- 左键单击：每次只播放一个约 0.8–1.2 秒的短动作，随后立即回到睡觉待机，不会再把所有动作串成一条长动画。
- 8 个点击动作按“打哈欠 → 委屈 → 奔跑 → 欢呼 → 点赞 → 吃饼干 → 挥手 → 思考”的顺序轮换；第 9 次有效点击重新从打哈欠开始。
- `pic` 文件夹中的 9 张原图会跨多次点击全部用上：`小鲁班2.png` 提供固定睡觉待机，其余 8 张分别对应 8 个短动作。
- 每个短动作使用专属 idle 桥接图从睡觉姿势平滑进入动作，并沿桥接图返回待机；状态之间采用 96ms 双图层交叉淡入，动作图在淡入完成后再按自身节奏停留。
...
8 个点击动作分别使用 `idle-to-yawn`、`idle-to-cry`、`idle-to-run`、`idle-to-cute`、`idle-to-like`、`idle-to-eat`、`idle-to-wave`、`think-to-idle` 作为专属待机桥接图。委屈、点赞、吃饼干和挥手等姿势跨度较大的动作还会短暂经过一张内部过渡图，但每次点击始终只表达一个动作并返回 `luban-idle.png`。全部运行时资源均为 `900×1100 RGBA` 透明 PNG，人物底部锚点统一，并使用高质量缩放。
