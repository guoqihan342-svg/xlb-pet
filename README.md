...
- 待办和定时任务正文均最多输入 5000 字，全文悬停规则与待办一致。单击定时任务右侧铅笔会打开固定约 `10cm × 5.5cm` 的橘色独立编辑窗，优先放在面板左侧、空间不足时回退右侧；正文、日期、时、分、秒和循环规则一次回填、一起修改，内容较多时只在窗内平滑滚动。“确定修改”或点击窗外只提交一次并保留任务身份，然后重新排序和调度；`Esc` 取消且不会误改原任务。待提醒列表自动保存到 `%LocalAppData%\LubanDesktopPet\scheduled-tasks.json`。
