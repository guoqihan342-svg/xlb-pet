...
static void CheckAppSettingsStore(string tempDirectory)
{
    ...
    Assert(!defaults.KeepTodoWindowOpenOnOutsideClick,
        "设置文件缺失时任务小屋必须默认允许外部点击自动收起");
    ...
    Assert(!firstLoaded.KeepTodoWindowOpenOnOutsideClick,
        "默认不固定任务小屋应能往返保存和加载");
    ...
    Assert(firstJson.Contains(
            "\"keepTodoWindowOpenOnOutsideClick\"",
            StringComparison.Ordinal),
        "设置 JSON 必须持久化 camelCase 的任务小屋固定字段");
    ...
    Assert(store.Save(new AppSettings
    {
        ...
        KeepTodoWindowOpenOnOutsideClick = true,
        PetSizeScale = 0.75
    }), "覆盖保存开启绕屏、固定任务小屋和尺寸下限应成功");
    var secondLoaded = store.Load();
    Assert(secondLoaded.EdgeRoamingEnabled, "开启绕屏动画应能往返保存和加载");
    Assert(secondLoaded.KeepTodoWindowOpenOnOutsideClick,
        "固定任务小屋应能往返保存和加载");
    ...
    File.WriteAllText(
        settingsPath,
        "{\"edgeRoamingEnabled\":false,\"alwaysOnTop\":true,\"petSizeScale\":1.1}");
    var legacySettings = store.Load();
    Assert(!legacySettings.EdgeRoamingEnabled,
        "旧版 JSON 中关闭的绕屏开关应继续保留");
    Assert(!legacySettings.KeepTodoWindowOpenOnOutsideClick,
        "旧版 alwaysOnTop=true 绝不能迁移成任务小屋默认固定");
    ...
    Assert(migratedJson.Contains(
               "\"keepTodoWindowOpenOnOutsideClick\": false",
               StringComparison.Ordinal) &&
           !migratedJson.Contains("\"alwaysOnTop\"", StringComparison.Ordinal),
        "旧版 JSON 保存后应改用默认关闭的新固定字段，不能沿用错误的置顶语义");
    ...
    Assert(!missingRoamingSetting.KeepTodoWindowOpenOnOutsideClick,
        "缺少固定字段的 JSON 应平滑迁移为默认不固定");
    ...
    Assert(!corruptedFallback.KeepTodoWindowOpenOnOutsideClick,
        "损坏 JSON 应回退到默认不固定任务小屋");
